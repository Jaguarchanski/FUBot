# main.py — webhook для Render, повний функціонал
import asyncio
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        # Вибір мови для нового користувача
        keyboard = [
    ...
    if plan == "FREE" and get_early_bird_count() < 500:
        ...
        await update.message.reply_text(get_text(lang, 'start_message'),
...
async def select_language(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    add_or_update_user(user_id, {"language": lang_code, "plan": "FREE"})
    await query.edit_message_text(get_text(lang_code, 'start_message'),
                                  reply_markup=main_menu(lang_code, "FREE"))
...
async def account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"Тариф: {plan}\nІнтервал: {user.get('interval', 'N/A')} хв\nПоріг: {user.get('threshold', 'N/A')}%\nБіржа: {user.get('exchange', 'N/A')}\nPRO до: {expires_str}"
